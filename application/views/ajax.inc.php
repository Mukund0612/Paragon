...
if(count($_SESSION['cart']['item_name']) == 0 )
{
	unset($_SESSION['order_id']);
}

if( !isset($_SESSION['order_id']) && empty($_SESSION['order_id']) ) { 
	$_SESSION['order_id'] = rand(10,99).time().rand(10,99); 
...

{
	...
	case 'add_to_cart':
			...
			$res_get_access = $this->db->query("SELECT * FROM access WHERE id = ".$_REQUEST['pro_id'])->result();


			if( $res_get_access[0]->a_price = $_REQUEST['pro_price']){
				
				$_SESSION['cart']['item_name'][]= $res_get_access[0]->a_name;

				$_SESSION['cart']['item_price'][]= $res_get_access[0]->a_price;

				$_SESSION['cart']['item_qty'][]= 1;

				$_SESSION['cart']['item_id'][]= $res_get_access[0]->id;
				
				$ins_data['order_id'] = $order_id;
				$ins_data['item_id'] = $res_get_access[0]->id;
				$ins_data['item_qty'] = 1;
				$ins_data['item_name'] = $res_get_access[0]->a_name;
				$ins_data['item_price'] = $res_get_access[0]->a_price;
				$ins_data['added_date'] = date('Y-m-d H:i:s');

				$insTemp = $this->Paramodel->insert('temp_order_item',$ins_data);
	...
	case 'update_qty':
			...
			if( $this->session->userdata('cart') && count($this->session->userdata('cart')['item_id'])>0){



					if (in_array($_REQUEST['pro_id'], $this->session->userdata('cart')['item_id'])){



						$key = array_search($_REQUEST['pro_id'], $this->session->userdata('cart')['item_id']);
						
						$_SESSION['cart']['item_qty'][$key] = $_REQUEST['pro_qty'];
						
						$up_temp_data['item_qty'] = $_REQUEST['pro_qty'];
						$wh_temp_data['order_id'] = $order_id;
						$wh_temp_data['item_id'] = $_REQUEST['pro_id'];
						
						$update = $this->Paramodel->update('temp_order_item',$up_temp_data,$wh_temp_data);
	...
	case 'delete_item':
			...
			if($this->session->userdata('cart') && count($this->session->userdata('cart'))>0){
					...
					if (in_array($_REQUEST['pro_id'], $this->session->userdata('cart')['item_id'])){
						...
						$wh_temp_del['order_id'] = $order_id;
						$wh_temp_del['item_id'] = $_REQUEST['pro_id'];

						$this->Paramodel->delete('temp_order_item',$wh_temp_del);
